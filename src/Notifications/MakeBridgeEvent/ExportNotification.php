...
namespace Innoboxrr\MakeBridge\Notifications\MakeBridgeEvent;

use Maatwebsite\Excel\Facades\Excel;
use Innoboxrr\MakeBridge\Exports\MakeBridgeEventsExports;
...
class ExportNotification extends Notification
{
    ...
    {
        return config('innoboxrrmakebridge.notification_via', ['mail', 'database']);
    }
    ...
    {
        Excel::store(
            new MakeBridgeEventsExports($this->data), 
            $this->path, 
            config('innoboxrrmakebridge.export_disk', 's3')
        );
    }

    private function getSubject()
    {
        return env('APP_NAME') . ' | MakeBridgeEvent export notification';
    }
